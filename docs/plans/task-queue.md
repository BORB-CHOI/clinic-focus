...
> 2026-05-24 추가. 강사 권한 대기 항목(KB DataSource S3)을 빼고 **지금 바로 진행 가능한 것**만 추린 작업 큐 + 의존성 그래프는 별도 문서로 분리:
>
> → **[`docs/plans/be-aws-wiring.md`](be-aws-wiring.md)**
>
> 13개 작업(B1~B13)을 권한 상태·의존성·추정 시간으로 정리. 권장 진행 순서·비용 추정·결정 필요 사항 포함.
>
> 핵심 가능 항목: DynamoDB 7테이블 생성(B1), 서울 5개구 1만 메타 적재(B3, HIRA Key 받은 후), 자체 S3 버킷 생성·`S3Adapter` boto3 전환(B4·B5), 풀크롤링 1만건(B7), 잡음 정제(B8), hash 컬럼(B9). 아래 PR 큐 #3·#6·#7과 매핑됨.
