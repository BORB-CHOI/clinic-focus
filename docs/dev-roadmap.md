...
개인 계정 Sonnet 4.5(Vision 시연용)는 별도 자격증명으로 호출한다.
...
| **벡터 저장·검색** | **S3 Vectors** (`bedrock-knowledge-base-1tvot3`) | 지원 | 병원 분류·설명 임베딩. 직접 `PutVectors`/`QueryVectors` 호출 |
| **LLM·Vision (시연 10개)** | Bedrock Haiku 4.5 또는 Nova | 지원 | 트랙 B 자칭 추출·`generate_description` (강사 자원, 10개 한도) |
| **Vision 고품질 시연 (10개)** | Bedrock Claude Sonnet 4.5 | 개인 | 트랙 C 이미지 분석 (한국어 OCR + 시각 해석) |
...
**스택**: Python 3.11 · boto3 (bedrock-runtime, s3vectors) · Pydantic · `shared/models.py`
...
병원 사이트에 올라온 의료기기 사진·시술 결과 사진을 개인 계정 Claude Sonnet 4.5의 vision 입력으로 분석 (트랙 C, 10개 한정). "사마귀 냉동치료기" "라식 장비" "MRI" 같은 보유 장비 식별, 시술 사진 분포(미용 vs 일반 진료) 추출. 이미지 안 글자(의료기기 신고증·가격표 등)도 같은 Vision 호출로 OCR 처리 — Textract는 한국어 미지원으로 사용하지 않는다. PoC는 정확도 완성보다 원리 검증이 목표.
...
Phase 1에서 S3에 적재한 이미지 URL을 EC2 batch로 처리. 의료기기 신고증·시술 사진·기기 사진 모두 개인 계정 Bedrock Claude Sonnet 4.5 vision 한 번 호출로 OCR + 시각 해석을 같이 받는다 (Textract 미사용). 결과는 DynamoDB에 적재. PoC 단계에서는 시연 10개 병원에 한정.
