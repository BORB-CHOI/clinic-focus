...
**개인 계정**(AI·ML 서비스 전담). EC2 코드는 지원 계정 서비스는 인스턴스 프로파일로,
개인 계정 서비스(Bedrock·S3 Vectors·Textract)는 개인 계정 자격증명으로 호출한다.
...
| **벡터 저장·검색** | **S3 Vectors** | 개인 | 병원 분류·설명 임베딩. RAG 검색의 1차 매칭 |
| **LLM·Vision** | Bedrock Claude Sonnet 4.5 (`us.` 인퍼런스 프로파일) | 개인 | 자칭 컨셉 추출, 분류 판정, RAG 답변, 사진 인식 |
| **OCR** | Amazon Textract | 개인 | 의료기기 신고증·시술 안내 텍스트 추출 |
| **임베딩** | Bedrock Titan Embed Text v2 | 개인 | 병원 설명·쿼리 임베딩 |
...
Phase 1에서 S3에 적재한 이미지 URL을 EC2 batch로 처리. 의료기기 신고증 같은 정형 텍스트는 Amazon Textract, 시술 사진·기기 사진 같은 비정형 이미지는 Bedrock Claude Sonnet 4.5 vision 호출. 결과는 DynamoDB에 적재.
...
**EC2 cron 기반 자동 갱신**  
크롤링 → 변경 감지 → AI 분석 → DB 적재의 전체 흐름을 EC2의 cron으로 자동화. 단계가 길면 SQS로 분리. 실패 알림은 SNS로 메일 발송.
