...
| 프레임워크 | **FastAPI + Mangum** | 자동 OpenAPI 스펙, Lambda 어댑터 |
| 데이터 검증 | **Pydantic** | FastAPI 기본 포함, AI 모듈과 모델 공유 |
| AWS SDK | boto3 | DynamoDB·S3·S3 Vectors·Bedrock·Textract |
| 크롤링 | **httpx + BeautifulSoup4** | 단순. JS 렌더링 필요한 일부만 Playwright 보강 |
| 배포 | **AWS SAM** | zip 배포, 도커 불필요 |
...
| AI 호출 | boto3 (bedrock-runtime, textract) | LLM·Vision·OCR 모두 단일 SDK |
| 벡터 | boto3 (s3vectors) | PutVectors / QueryVectors 직접 호출 |
...
- **PDF 의료기기 신고증** → PyMuPDF (Textract 실패 시)
...
| 계층 | 서비스 | 용도 |
|---|---|---|
| **컴퓨팅** | AWS Lambda | API 핸들러, 크롤링·분류 배치. 도커 없이 zip 또는 SAM으로 배포 |
| **API 게이트웨이** | API Gateway (HTTP API) | 프론트 ↔ Lambda 라우팅. 인증 없음, public endpoint |
| **정형 데이터** | DynamoDB | 병원 / 분류 결과 / 시그널 / 신뢰도 / 피드백 / 변경 이력 |
| **원본 저장** | S3 (일반 버킷) | 크롤링한 HTML·이미지 원본 |
| **벡터 저장·검색** | **S3 Vectors** | 병원 분류·설명 임베딩. RAG 검색의 1차 매칭. Bedrock과 직접 통합 |
| **LLM** | Bedrock Claude Sonnet 4.5 | 자칭 컨셉 추출, 분류 판정, RAG 답변 |
| **Vision** | Bedrock Claude Sonnet 4.5 (vision) | 의료기기·시술 사진 인식 |
| **OCR** | Amazon Textract | 의료기기 신고증·시술 안내 텍스트 추출 |
| **임베딩** | Bedrock Titan Embed Text v2 | 병원 설명·쿼리 임베딩 |
| **프론트 호스팅** | S3 + CloudFront | 정적 빌드 결과물 배포 |
| **CDN** | CloudFront | 프론트 가속 (CI·CD·로드밸런싱 없음) |
| **배포** | 수동 (AWS CLI / SAM) | CI/CD 없음 |

---

## 최비성 트랙 — AI · RAG

**스택**: Python 3.11 · boto3 (bedrock-runtime, s3vectors, textract) · Pydantic · `shared/models.py`

### Phase 1 (M0~3) — PoC

**자칭 컨셉 추출 프롬프트 설계**  
김경재가 크롤링한 사이트 텍스트를 Bedrock Claude Sonnet 4.5에 던져서 그 병원이 어떤 진료 분야를 메인으로 내세우는지 태그로 뽑아낸다. 예: 정형외과 사이트인데 메인 페이지에 어깨·견관절 시술 사진이 70%, 척추 언급 0% → "어깨 전문" 태그. 과목별 프롬프트 템플릿 분리 (피부과·정형외과·이비인후과·안과 등).

**Bedrock Vision 의료기기·시술 사진 인식 PoC**  
병원 사이트에 올라온 의료기기 사진·시술 결과 사진을 Bedrock Claude Sonnet 4.5의 vision 입력으로 분석. "사마귀 냉동치료기" "라식 장비" "MRI" 같은 보유 장비 식별, 시술 사진 분포(미용 vs 일반 진료) 추출. 의료기기 신고증 같은 정형 텍스트는 Amazon Textract OCR로 추출. PoC는 정확도 완성보다 원리 검증이 목표.
...
4 시그널 데이터(자칭·Vision·블로그·후기) + 분류 결과 + 신뢰도 점수를 Bedrock Claude Sonnet 4.5에 입력해 병원의 정체성·실제 진료 영역·강점·약점·주의사항을 **자연어 단락**으로 생성한다. 모든 문장에 출처 시그널 태그(`[사이트]`, `[Vision]`, `[블로그]`, `[후기]`)를 자동 부착해 추적 가능성 확보. 의료법 회색지대 회피를 위해 항상 주체 명시 표현 사용("이 병원이 자기 사이트에서 ~를 메인으로 표시함"). 프롬프트 템플릿은 별도 파일로 관리하며 과목별로 분리. 출력 결과는 DynamoDB에 저장돼 상세 페이지 API가 그대로 반환. **기존 서비스(굿닥·모두닥·똑닥)와의 가장 큰 차별점이 시각화되는 지점.**
...
**자연어 검색 UI**  
검색창에 자연어 질의 가능. 예: "M자 탈모 처방받을 수 있는 동네 의원" → API Gateway → Lambda → S3 Vectors 검색 → 결과 표시. 자동 완성·인기 검색어·최근 검색 기록(localStorage)은 기본 UX.
...
**스택**: Python 3.11 · FastAPI + Mangum (Lambda 어댑터) · Pydantic · boto3 · httpx + BeautifulSoup4 (크롤링) · AWS SAM (배포)

### Phase 1 (M0~3) — PoC

**병원 사이트 크롤러 (Lambda)**  
PoC 대상 서울 5개 구 약 1만 병원 사이트 자동 수집. `httpx + BeautifulSoup4`로 한 사이트당 메인·소개·진료 안내·의료진 등 핵심 5~10 페이지의 HTML·이미지 URL 추출 → S3에 원본 적재. robots.txt 준수, User-Agent 명시, 요청 간격 조절. JS 렌더링 필요한 사이트는 Playwright Lambda Layer로 보강. Lambda 단일 함수 15분 한도가 문제되면 SQS + Lambda fan-out 또는 EC2 단일 인스턴스 batch로 처리.

**심평원 공공 API 통합**  
건강보험심사평가원 공공 데이터 포털에서 의료기관 기본 정보·전문의 자격·신고된 의료기기 목록 수집. 합법·무료. 콜드 스타트(신규·지방 병원 데이터 빈약) 완화 + 크롤링 보조.

**DynamoDB 스키마 설계**  
핵심 테이블: Hospitals / Classifications / Signals / Confidence / Feedback / ChangeHistory. 최비성의 분류 스키마 v1을 파티션 키·정렬 키·GSI 구조로 반영. 위치 기반 검색은 GeoHash GSI 또는 S3 Vectors 메타데이터 필터로 처리.

**FastAPI + Mangum 기반 API 서버**  
FastAPI로 작성한 라우터를 Mangum 어댑터로 감싸 Lambda에 배포. 검색 API, 병원 상세 API, 분류 변경 이력 API, 피드백 제출 API 네 개 엔드포인트. 자동 생성되는 OpenAPI 스펙(`/openapi.json`)은 프론트가 TS 타입 자동 생성에 그대로 사용. 인증 없음 — public endpoint, 호출 제한은 API Gateway throttling 기본값.

**프론트 호스팅 (S3 + CloudFront)**  
하재원 빌드 결과물을 S3에 업로드, CloudFront로 배포. 도메인 연결은 Route 53(선택) 또는 CloudFront 기본 도메인 사용. 수동 배포(`aws s3 sync` + `aws cloudfront create-invalidation`).

### Phase 2 (M3~6) — 베타

**Textract + Bedrock Vision 파이프라인**  
Phase 1에서 S3에 적재한 이미지 URL을 Lambda batch로 처리. 의료기기 신고증 같은 정형 텍스트는 Amazon Textract, 시술 사진·기기 사진 같은 비정형 이미지는 Bedrock Claude Sonnet 4.5 vision 호출. 결과는 DynamoDB에 적재.

**S3 Vectors 인덱스 구축**  
최비성이 정의한 분류 결과·설명 텍스트를 Titan Embed Text v2로 임베딩 → S3 Vectors의 vector index에 PutVectors API로 적재. 메타데이터로 지역·진료과목·세부 주력·신뢰도 점수 + **위경도(lat, lng)** 를 함께 저장해 쿼리 단계에서 필터링 가능.

**지오 검색 API (`/api/search/nearby`)** ⭐  
사용자 현재 위경도 + 반경(km)을 받아 반경 내 병원 리스트를 반환. 구현:
1. 입력 위경도 기준 bounding box 계산 (반경 5km면 위경도 ±0.045°)
2. S3 Vectors 메타데이터 필터로 1차 후보 추출 (bounding box 내 lat/lng)
...
**변경 감지(diff) 시스템**  
분기 1회 재크롤링 시 이전 데이터와 비교해서 변경된 페이지만 다시 AI 분석. 전체 재처리 대비 비용 80~90% 절감. 변경 이력 DB 자동 기록. 평가 단계에서는 수동 트리거로 충분.
...
**EventBridge 기반 자동 갱신**  
크롤링 → 변경 감지 → AI 분석 → DB 적재의 전체 흐름을 EventBridge 스케줄 룰로 자동화. Lambda 체이닝 + SQS로 단계 분리. 실패 알림은 SNS로 메일 발송.

**인간 검수자 어드민 백엔드**  
하재원의 검수자 어드민 페이지가 호출할 API. 분류 대기열·시그널 원본·피드백 누적 통계 조회 가능. 인증은 IAM이나 Cognito 대신 Lambda Function URL + AWS Signature 또는 단순 API Key (평가 단계).
...
| **문제 해결 타당성 (25%)** | 4 시그널 교차 검증 + S3 Vectors RAG + 신뢰도 점수 + 익명 피드백 루프 |
| **창의성 (25%)** | ⭐ **AI가 4 시그널을 종합해 생성한 자연어 통합 상세 설명** (`generate_hospital_description`) — 기존 서비스가 안 하는 핵심 차별점. 추가로 자기 정체성 색인 + Bedrock Vision + S3 Vectors 활용 + 주체 명시 표현(의료법 회색지대 회피) |
