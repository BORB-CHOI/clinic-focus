...
두 모드:
  ...
  demo=True (시연 약 500개) — classify(use_llm=True) + 진료항목·설명·관련병원(LLM/Vision)
...
def run_index_pipeline(
    ...
    *,
    ...
    Args:
        hospital_id: 대상 병원.
        demo: True 면 LLM/Vision 까지 (시연 약 500개). False 면 룰 단독 베이스라인.
    ...
    # Vision 결과 로드 — DDB VISION#RESULTS 가 있으면(=시연 약 500개 demo) 합류.
    ...
    # Vision 결과가 있으면 전달 → 분류기가 재분석하지 않고 재사용 (시연 약 500개).
