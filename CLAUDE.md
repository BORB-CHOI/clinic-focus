...
| 항목 | 값 | 계정 | 사용 범위 |
|---|---|---|---|
| **검색 재랭커(런타임)** | `anthropic.claude-3-haiku-20240307-v1:0` (on-demand) / `RERANK_MODEL_ID` 로 Nova 교체 | 지원 | `RERANK_MODE=llm` 일 때 검색당 1회 |
...
> ★ 지원 계정 on-demand 만 호출 가능(실측 2026-06-08): **Claude 3 Haiku·Nova ✅** / Haiku 4.5·Sonnet·모든 `us.`·`global.` inference profile **❌ AccessDenied/Validation**. 그래서 런타임 LLM(재랭커)은 Claude 3 Haiku 기본.
...
| **D. 검색 재랭커 (런타임)** | 2-stage RAG 2단계 정렬 | **지원 Claude 3 Haiku (on-demand)** | `RERANK_MODE=llm` |
