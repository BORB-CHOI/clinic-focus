...
├── ai/         # 최비성 — 룰 기반 분류(서울 1만) + Bedrock LLM/Vision 시연(10개) + Bedrock Knowledge Base (강사 제공 `kmuproj-team-03`) + Titan Embed v2
...
- **DynamoDB 테이블 / S3 버킷은 콘솔 수동 생성.** 절차는 [`docs/setup/aws-onboarding.md`](docs/setup/aws-onboarding.md) Step 6 (DDB) · 별도 S3 섹션. 스키마 명세는 [`docs/plans/task-queue.md`](docs/plans/task-queue.md) §3.
...
| Vision (고품질 시연) | `global.anthropic.claude-sonnet-4-6` (Global cross-region inference profile, 서울 리전 호출) | 개인 | 10개 병원 (시연용) |
...
| C. Vision 시연 | 이미지 분석 | 개인 Sonnet 4.6 | 10개 |
