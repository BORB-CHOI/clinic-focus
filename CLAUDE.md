...
| 계정 | 서비스 | 리전 | 자격증명 |
|---|---|---|---|
| **지원 계정** | EC2 · RDS · DynamoDB · S3 · **Bedrock Knowledge Base (`kmuproj-team-03`, 강사 제공)** · Bedrock(Titan Embed v2, Haiku/Nova 한정) · API Gateway · Amplify · SQS · SNS | `us-east-1` | IAM Role만 (Access Key 발급 불가). EC2=`SafeInstanceProfile-{username}` |
| **개인 계정** | Bedrock (Claude Sonnet 4.6 — Vision 시연 한정, Global cross-region inference profile) | `ap-northeast-2` | 개인 계정 자격증명 (전용 프로파일/환경변수) |
...
- **EC2 코드의 자격증명 분리.** 지원 계정 서비스(DynamoDB·S3·Bedrock KB·Titan Embed·Haiku/Nova)는 EC2 인스턴스 프로파일로 자동 인증. **개인 계정 Sonnet 4.6(Vision 시연용)** 만 별도 자격증명으로 boto3 클라이언트를 따로 생성한다 (서울 리전 + Global inference profile).
- **지원 계정 Bedrock 제약(중요).** 강사가 제공한 자원이라 **모델이 한정**된다 — 텍스트/Vision **Haiku 또는 Nova만**(개인 계정 Sonnet 4.6은 Vision 시연용). 병원 수 하드 한도는 아니고, 실제 시연은 LLM·Vision 모두 **약 500개 병원**(실측 DESCRIPTION 504·VISION 508)에 적용했다. 임베딩(Titan v2)은 전체 자유. 전체 1만 병원 텍스트 분류는 **룰 기반(LLM 미사용)** 으로 처리.
...
| 항목 | 값 | 계정 | 사용 범위 |
|---|---|---|---|
| LLM/Vision (시연) | `anthropic.claude-haiku-4-5-...` 또는 `amazon.nova-...` | 지원 | 약 500개 병원 (실측 504) |
| Vision (고품질 시연) | `global.anthropic.claude-sonnet-4-6` (Global cross-region inference profile, 서울 리전 호출) | 개인 | 약 500개 병원 (실측 508) |
...
| B. LLM 텍스트 시연 | 자칭 추출 + `generate_description` | 지원 Haiku/Nova | 약 500개 (504) |
| C. Vision 시연 | 이미지 분석 | 개인 Sonnet 4.6 | 약 500개 (508) |

→ A는 1만 풀커버 베이스라인, B·C는 같은 약 500개 병원으로 차별 효과 시연. 자세한 건 `ai/CLAUDE.md`.
