...
# 외부 제3자 후기 블로그(카카오 place앵커 blog)는 별도 '블로그 시그널' 이라 여기 포함 안 함
...
def _kakao_blog_text(kakao_blog) -> str:
    """카카오 place앵커 blog seeds 의 title + contents 를 합쳐 반환 (키워드 추출 입력).

    kakao_blog 는 KakaoBlog 모델 또는 parse_blog() dict (`seeds[]`). 없으면 "".
    네이버 키워드 검색(naver_blog)과 달리 place_id 에 묶인 블로그라 호스트 병원 앵커
    정밀도가 압도적이다(실측 동일URL 교차오염 0.75% vs 네이버 16.78%, 22배). 그래서
    블로그 시그널의 권위 소스로 카카오 place앵커를 쓴다. 작성자 PII 는 parse_blog 에서 제거됨.
    """
    kb = _as_dict(kakao_blog)
    if not kb:
        return ""
    parts: list[str] = []
    for seed in (kb.get("seeds") or []):
        if isinstance(seed, dict):
            title, contents = seed.get("title") or "", seed.get("contents") or ""
        else:
            title, contents = getattr(seed, "title", ""), getattr(seed, "contents", "")
        if title or contents:
            parts.append(f"{title} {contents}".strip())
    return "\n".join(parts)
...
def _analyze_blog(
    ...
    **카카오 place앵커 blog(kakao_blog)** 를 권위 소스로 본다 — 네이버 키워드 검색
    (naver_blog)은 호스트 병원 앵커가 느슨해(교차오염 16.78%) 블로그 시그널에서 폐기.
    자체 blog 페이지는 자기가 쓴 것이라 자칭(self_claim)으로 귀속(저자 기준 분리).
    naver_blog 인자는 하위호환용(미사용) — 네이버는 향후 place앵커 후기로만 합류.
    """
    blog_text = _kakao_blog_text(kakao_blog)
    if not blog_text:
        return BlogSignal(total_posts=0, keyword_frequency={}, primary_topics=[])

    combined_text = blog_text
...
def classify_hospital(
    ...
    *,
    ...
    Args:
        ...
        kakao_blog: KakaoBlog 모델 또는 parse_blog() dict. **블로그 시그널의 권위 소스**
            (place앵커, 교차오염 0.75%). naver_blog(키워드 검색)는 노이즈로 폐기.
