...
| `ai-engineer` | `ai/` + `shared/` | Bedrock + Bedrock Knowledge Base. 분류·생성·검색(KB Retrieve)·Vision·시그널 융합·임베딩·KB ingestion |
...
| `ingest_hospital` | vector-search-engineer | Bedrock KB DataSource S3 업로드 + ingestion job (sido/sigungu/lat/lng metadata 포함) |
| `retrieve_hospital` | vector-search-engineer | KB Retrieve 자연어 검색 (수동 카테고리 탐색은 BE DynamoDB GSI로 분리) |
